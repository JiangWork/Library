...
public class Proc {
	...
	private ProcResult runInternal() throws InterruptedException, IOException {
		...
		if (!done) { // runner is still running
			...
			throw new ProcExecutionException("Timeout after waiting " + timeOut + " ms" ,procName);
		}
		runner.join();
		return result;
	}
	...
	public ProcResult run() {
		try {
			...
			throw new ProcExecutionException(e.getMessage(), procName);
		}
	}
	...
	public class CommandRunner extends Thread
	{
		...
		public void run() {
			...
			try {
				...
				throw new ProcExecutionException(e.getMessage(), "CommandRunner", e);
			} finally {
				lock.release();
				result.setEndTime(System.currentTimeMillis());
		...
		public void terminate() {
			System.out.println("Terminate the command runner.");
			stopCopier(inCopier);
	...
	public class StreamCopier extends Thread
	{
		...
		public void blockingCopy() { 
			...
			try {
				...
				throw new ProcExecutionException(e.getMessage(), "StreamCopier");
			} finally {
				try {
					is.close();
					os.close();
	...
	public static class ProcBuilder {
		...
		public Proc build() {
			if (command == null) {
				throw new ProcExecutionException("Null command.", procName);
			}
			if (workingDirectory != null) {
				File file = new File(workingDirectory);
				if (!file.exists()) {
					throw new ProcExecutionException("No such working directory:" + workingDirectory,
							procName);
				}
			}
